...
pub async fn fetch_response(resource: &str) -> Result<Response> {
    fetch_with_str(resource)
        .await?
        .dyn_into()
        .map_err(|err| anyhow!("Could not convert to Response {:#?}", err))
}
...
pub async fn fetch_array_buffer(resource: &str) -> Result<ArrayBuffer> {
    let array_buffer = fetch_response(resource)
        .await?
        .array_buffer()
        .map_err(|err| anyhow!("Could not get array buffer {:#?}", err))?;
    ...
    if let Some(child) = ui.first_child() {
        ui.remove_child(&child)
            ...
            .and_then(|_unit| {
                canvas()?
                    .focus()
                    .map_err(|err| anyhow!("Could not focus canvas {:#?}", err))
            })
    } else {
        Ok(())
    }
...
mod tests {
    ...
    async fn test_error_loading_json() {
        let json = fetch_json("not_a_real_file.json").await;
        assert!(json.is_err());
    }
}
