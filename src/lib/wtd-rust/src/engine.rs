...
fn prepare_input() -> Result<UnboundedReceiver<KeyPress>> {
    ...
    let onkeydown = browser::closure_wrap(Box::new(move |keycode: web_sys::KeyboardEvent| {
        ...
        {
            log!("Error sending keydown event: {:#?}", err);
        }
    ...
    let onkeyup = browser::closure_wrap(Box::new(move |keycode: web_sys::KeyboardEvent| {
        ...
        {
            log!("Error sending keyup event: {:#?}", err);
        }
    }) as Box<dyn FnMut(web_sys::KeyboardEvent)>);
...
pub fn add_click_handler(elem: HtmlElement) -> UnboundedReceiver<()> {
    ...
    let on_click = browser::closure_wrap(Box::new(move || {
        click_sender
            .start_send(())
            .expect("Could not add click handler");
    }) as Box<dyn FnMut()>);
...
mod tests {
    ...
    fn two_rects_that_intersect_on_the_left() {
        let rect1 = Rect::new_from_x_y(0, 0, 10, 10);
        let rect2 = Rect::new_from_x_y(5, 0, 10, 10);
        assert!(rect1.intersects(&rect2));
    }

    #[test]
    fn two_rects_that_intersect_on_the_right() {
        let rect1 = Rect::new_from_x_y(5, 0, 10, 10);
        let rect2 = Rect::new_from_x_y(0, 0, 10, 10);
        assert!(rect1.intersects(&rect2));
    }
    ...
    fn two_rects_that_intersect_on_the_top() {
        let rect1 = Rect::new_from_x_y(0, 0, 10, 10);
        let rect2 = Rect::new_from_x_y(0, 5, 10, 10);
        assert!(rect1.intersects(&rect2));
    }

    #[test]
    fn two_rects_that_intersect_on_the_bottom() {
        let rect1 = Rect::new_from_x_y(0, 5, 10, 10);
        let rect2 = Rect::new_from_x_y(0, 0, 10, 10);
        assert!(rect1.intersects(&rect2));
    }
    ...
    fn two_rects_that_do_not_intersect() {
        let rect1 = Rect::new_from_x_y(0, 0, 10, 10);
        let rect2 = Rect::new_from_x_y(11, 11, 10, 10);
        assert!(!rect1.intersects(&rect2));
    }
}
